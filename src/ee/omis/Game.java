...
public class Game {
        ...
        while (chosenItem == null) {
            try {
                ...
                if(finalChosenItem.getName().equals("Teleporter")) {
                    ...
                    checkCharacterCoordinates(player, enemy, questMaster, healer, false);
                    System.out.println("TELEPORTISID");
                } else {
                ...
                System.out.println("SEDA VIGA EI TOHI TEKKIDA");
            }
        }
    }
    ...
    private static void startBattle(Map<EnemyType, Integer> defeatedEnemies, Player player, Enemy enemy, Healer healer, QuestMaster questMaster, Scanner scanner, String input, Item finalChosenItem) throws InterruptedException {
        while (player.getHealth() > 0 && enemy.getHealth() > 0) {
            ...
            System.out.println("ÜTLE NUMBER ÜHEST KOLMENI JA KUI PANED TÄPPI, SIIS VÕIDAD VASTASE");
            ...
            if (randomNumber == Integer.parseInt(userEntered)) {
                enemyHit(defeatedEnemies, player, enemy, healer, questMaster, (FightWeapon) finalChosenItem);
            } else {
    ...
    private static void playerHit(Map<EnemyType, Integer> defeatedEnemies, Player player) throws InterruptedException {
        ...
        if(player.getHealth() <= 0) {
            ...
            if(player.getLives() < 1) {
                ...
                System.out.println("SAID SURMA! OTSI RAVITSEJA ÜLESSE!");
            }
        }
    }

    private static void enemyHit(Map<EnemyType, Integer> defeatedEnemies, Player player, Enemy enemy, Healer healer, QuestMaster questMaster, FightWeapon finalChosenItem) throws InterruptedException {
        TimeUnit.MILLISECONDS.sleep(1000);
        System.out.println("ÕIGE! VÕTSID VASTASE ELU");
